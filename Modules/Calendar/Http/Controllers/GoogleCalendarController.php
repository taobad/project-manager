...
class GoogleCalendarController extends Controller
{
    ...
    public function importGoogleEvents(Request $request)
    {
        ...
        if (!is_null($code)) {
            ...
            $result = \JsonMachine\JsonMachine::fromString($googleService->request('https://www.googleapis.com/calendar/v3/calendars/primary/events?alt=json&max-results=400'));
    ...
    public function deleteCalendarEvent($event_id, $calendar_id, $access_token)
    {
        ...
        $req = curl_init();
        curl_setopt($req, CURLOPT_URL, $url_events);
        curl_setopt($req, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($req, CURLOPT_CUSTOMREQUEST, 'DELETE');
        curl_setopt($req, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($req, CURLOPT_HTTPHEADER, array('Authorization: Bearer ' . $access_token, 'Content-Type: application/json'));
        curl_exec($req);
