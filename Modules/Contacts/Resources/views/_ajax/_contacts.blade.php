  ...
  <div class="col-lg-3 col-md-6">
    <div class="thumbnail">
      ...
      <div class="caption text-center">
        <h6>
          <a class="text-indigo-600" href="{{ route('contacts.view', $contact->user_id) }}">
            {{ $contact->name }}
          </a>
        ...
        <p class="mt-3">
          ...
          <a href="{{ $contact->twitter }}" target="_blank"
            class="btn btn-rounded btn-twitter btn-icon">@icon('brands/twitter')</a>
          @endif
          @if(!empty($contact->skype))
          <a href="skype:{{ $contact->skype }}?call" class="btn btn-rounded btn-info btn-icon">@icon('brands/skype')</a>
          @endif
          <a href="{{ route('contacts.email', $contact->user_id) }}" class="btn btn-rounded btn-dracula btn-icon"
            data-toggle="ajaxModal" data-rel="tooltip" title="@langapp('send_email')">@icon('solid/paper-plane')</a>
