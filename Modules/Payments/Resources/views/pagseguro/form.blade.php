...
<form action="{{ route('pagseguro.form') }}" id="pagseguro-form" class="bs-example form-horizontal" method="POST">
    ...
    <div class="modal-footer"><a href="#" class="btn {{themeButton()}}" data-dismiss="modal">{{ langapp('close') }}</a>
        <button type="submit" id="pagseguro_submit" class="btn {{themeButton()}} pay-btn">{{ langapp('pay_invoice') }} via pagseguro</button>
    </div>
