...
<form action="https://www.paypal.com/cgi-bin/webscr" id="paypal-form" class="bs-example form-horizontal" method="POST">
    ...
    <form action="https://www.sandbox.paypal.com/cgi-bin/webscr" id="paypal-form" class="bs-example form-horizontal" method="POST">
        ...
        <div class="flex m-2">
            <img class="mr-1" src="{{ getStorageUrl(config('system.media_dir').'/payment_american.png') }}">
            <img class="mr-1" src="{{ getStorageUrl(config('system.media_dir').'/payment_discover.png') }}">
            <img class="mr-1" src="{{ getStorageUrl(config('system.media_dir').'/payment_maestro.png') }}">
            <img class="mr-1" src="{{ getStorageUrl(config('system.media_dir').'/payment_paypal.png') }}">
        </div>


        <div class="modal-footer"><a href="#" class="btn btn-default" data-dismiss="modal">{{ langapp('close') }}</a>
            <button type="submit" id="paypal_submit" class="btn btn-success pay-btn"><i class="fab fa-paypal"></i> {{ langapp('pay_invoice') }}</button>
        </div>
