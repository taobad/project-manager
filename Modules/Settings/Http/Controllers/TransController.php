...
class TransController extends Controller
{
    ...
    public function restore()
    {
        ...
        try {
            ...
            $jsonStream = \JsonMachine\JsonMachine::fromString($file);
            foreach ($jsonStream as $translations) {
    ...
    {
        $jsonStream = \JsonMachine\JsonMachine::fromString(html_entity_decode($this->request->json));
        foreach ($jsonStream as $jdata) {
            $jdata = (object)$jdata;
            if ($jdata->value != trans($this->request->filename . '.' . $jdata->name, [], $this->request->locale)) {
                $lng = [];
                foreach (languages() as $lang) {
                    $lng[$lang['code']] = $lang['code'] != $this->request->locale ? trans($this->request->filename . '.' . $jdata->name, [], $lang['code']) : $jdata->value;
                }
    ...
    {
        Language::whereCode($locale)->update(['active' => $this->request->active]);
        $data['message']  = langapp('changes_saved_successful');
        $data['redirect'] = '/settings/translations';
