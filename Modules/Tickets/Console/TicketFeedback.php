...
class TicketFeedback extends Command
{
    ...
    {
        foreach ($this->ticket->closed()->where('rated', 0)->where('feedback_disabled', 0)->get() as $tkt) {
            if (get_option('ticket_feedback_after') > 0) {
                if (today()->diffInDays($tkt->closed_at) == get_option('ticket_feedback_after')) {
                    \Mail::to($tkt->user)->send(new FeedbackRequestMail($tkt));
                }
            }
        }
