...
class TimerController extends Controller
{
    ...
    public function bill(TimeEntry $entry)
    {
        ...
        return redirect()->route('projects.view', ['project' => $entry->timeable->id, 'tab' => 'timesheets']);
    }
    ...
    public function tableData($project = null)
    {
        ...
        $model = $this->applyFilter($project->timesheets()->with('user:id,name'))->orderByDesc('id');

        return DataTables::eloquent($model)
            ->editColumn(
                'name',
                function ($entry) {
                    $str = '<a href="' . route('timetracking.view', $entry->id) . '" data-toggle="ajaxModal" class="' . themeLinks('text-ellipsis') . '">';
            ...
            ->editColumn(
                ...
                function ($entry) {
                    ...
                    if (isAdmin() || $entry->user_id == Auth::id()) {
                        if ($entry->billable == 0) {
                            $str .= '<a href="' . route('timetracking.bill', $entry->id) . '" data-toggle="tooltip" title="' . langapp('billable') . '" data-placement="left" class="m-r-sm">';
                            $str .= ' <i class="fas fa-clock text-success"></i></a>';
                        } else {
                            $str .= '<a href="' . route('timetracking.unbill', $entry->id) . '" data-toggle="tooltip" title="' . langapp('not_billable') . '" data-placement="left" class="m-r-sm">';
